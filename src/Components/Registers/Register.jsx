            ...
            if (!image) {
                ...
                getDocs(dataQuery).then((snapshot) => {
                    if (snapshot.docs.length === 0) {
                        uploadBytes(ref(storage, `images/${user.username}`), image).then((snap) => {
                            getDownloadURL(ref(storage, `images/${user.username}`)).then((url) => {
                                user.url = url;
                                addDoc(collection(database, "users_info"), user).then((docRef) => {
                                    setUser({
                                      username: '',
                                      fname: '',
                                      lname: '',
        ...
        <div className='container'>
            ...
            <div className="profile-photo">
                <div className="photo-container">
                    ...
                    <div id='setprofile'>
                        Add Profile Photo
                        <input type="file" onChange={(e) => { setImage(e.target.files[0]) }} className="hide_file" />
                    </div>
                </div>
            </div>
        </div>
